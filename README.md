...
В случае выгрузки из серверной информационной базы и использовании утилиты `ibcmd` дополнительно можно указать переменные среды:
...
* **V8_SRV_AGENT_PORT** - порт агента кластера серверов 1С:Предприятие
* **V8_RAS_PORT** - порт службы RAS (поднимается автоматически на машине запуска тестов)
