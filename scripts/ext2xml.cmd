...
echo ======
echo [INFO] Convert 1C configuration extension to 1C:Designer XML format
...
md "%LOCAL_TEMP%"
IF exist "%V8_DST_PATH%" IF "%V8_EXT_XML_CLEAN_DST%" equ "1" rd /S /Q "%V8_DST_PATH%"
...
IF "%V8_CONVERT_TOOL%" equ "designer" (
    ...
    IF defined V8_BASE_IB_SERVER (
        %IBCMD_TOOL% infobase config load --dbms=%V8_DB_SRV_DBMS% --db-server=%V8_BASE_IB_SERVER% --db-name="%V8_BASE_IB_NAME%" --db-user="%V8_DB_SRV_USR%" --db-pwd="%V8_DB_SRV_PWD%" --user="%V8_IB_USER%" --password="%V8_IB_PWD%" --extension=%V8_EXT_NAME% --force "%V8_SRC_PATH%"
    ) ELSE (
        %IBCMD_TOOL% infobase config load --db-path="%IB_PATH%" --user="%V8_IB_USER%" --password="%V8_IB_PWD%" --extension=%V8_EXT_NAME% --force "%V8_SRC_PATH%"
    )
)
...
IF "%V8_CONVERT_TOOL%" equ "designer" (
    set V8_DESIGNER_LOG=%LOCAL_TEMP%\v8_designer_output.log
    %V8_TOOL% DESIGNER /IBConnectionString %V8_BASE_IB_CONNECTION% /N"%V8_IB_USER%" /P"%V8_IB_PWD%" /DisableStartupDialogs /Out "!V8_DESIGNER_LOG!" /DumpConfigToFiles "%V8_DST_PATH%" -Extension %V8_EXT_NAME% -force
    FOR /F "tokens=* delims=" %%i IN (!V8_DESIGNER_LOG!) DO IF "%%i" neq "" echo [WARN] %%i
) ELSE (
    set IBCMD_EXPORT_FLAGS=--force
    IF exist "%V8_DST_PATH%\Configuration.xml" set IBCMD_EXPORT_FLAGS=!IBCMD_EXPORT_FLAGS! --sync
    IF defined V8_BASE_IB_SERVER (
        %IBCMD_TOOL% infobase config export --dbms=%V8_DB_SRV_DBMS% --db-server=%V8_BASE_IB_SERVER% --db-name="%V8_BASE_IB_NAME%" --db-user="%V8_DB_SRV_USR%" --db-pwd="%V8_DB_SRV_PWD%" --user="%V8_IB_USER%" --password="%V8_IB_PWD%" --extension=%V8_EXT_NAME% !IBCMD_EXPORT_FLAGS! "%V8_DST_PATH%"
    ) ELSE (
        %IBCMD_TOOL% infobase config export --db-path="%IB_PATH%" --user="%V8_IB_USER%" --password="%V8_IB_PWD%" --extension=%V8_EXT_NAME% !IBCMD_EXPORT_FLAGS! "%V8_DST_PATH%"
    )
)
