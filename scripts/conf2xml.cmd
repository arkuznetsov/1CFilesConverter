...
echo ======
echo [INFO] Convert 1C configuration to 1C:Designer XML format
...
IF "%V8_CONVERT_TOOL%" equ "designer" (
    ...
    %V8_TOOL% DESIGNER /IBConnectionString %V8_IB_CONNECTION% /N"%V8_IB_USER%" /P"%V8_IB_PWD%" /DisableStartupDialogs /Out "!V8_DESIGNER_LOG!" /DumpConfigToFiles "%V8_DST_PATH%" -force
    ...
    IF defined V8_IB_SERVER (
        %IBCMD_TOOL% infobase config export --dbms=%V8_DB_SRV_DBMS% --db-server=%V8_IB_SERVER% --db-name="%V8_IB_NAME%" --db-user="%V8_DB_SRV_USR%" --db-pwd="%V8_DB_SRV_PWD%" --user="%V8_IB_USER%" --password="%V8_IB_PWD%" !IBCMD_EXPORT_FLAGS! "%V8_DST_PATH%"
    ) ELSE (
        %IBCMD_TOOL% infobase config export --db-path="%IB_PATH%" --user="%V8_IB_USER%" --password="%V8_IB_PWD%" !IBCMD_EXPORT_FLAGS! "%V8_DST_PATH%"
    )
)
set ERROR_CODE=%ERRORLEVEL%
goto finally
